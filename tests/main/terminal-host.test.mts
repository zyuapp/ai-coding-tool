...
afterAll(() => { if (loginShell === undefined) delete process.env.SHELL; else process.env.SHELL = loginShell; });
...
async function until<T>(check: () => T | null | false | Promise<T | null | false>, message: string): Promise<T> {
...
const lines = (snapshot: TerminalText | null) => {
  assert.ok(snapshot);
...
test("a filtered read keeps only matching lines and counts them", async (t) => {
  ...
  const snapshot = await until(
    async () => {
      ...
      return (read?.matched ?? 0) >= 2 ? read : null;
    },
    "the shell never printed",
  );
