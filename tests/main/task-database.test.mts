...
test("SQLite automation storage keeps one row per task and drops unreadable rows", async () => {
  ...
  try {
    ...
    reopened.saveAutomation({ ...automation, id: "automation-3", taskId: "task-3", schedule: 42 } as unknown as Automation);
...
test("a project folder that merely starts with the worktrees root's name is still allowed", async () => {
  ...
  try {
    ...
    assert.deepEqual(loadDatabase(database).projects, [{ id: "project-1", root: `${worktreesRoot}-mine` }]);
  } finally {
    database.close();
    await rm(directory, { recursive: true, force: true });
  }
