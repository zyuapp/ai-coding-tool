...
test("a thread that has an engine offers only its models, and says a new thread is how to use the other", async () => {
  ...
  assert.ok([...modelMenu.querySelectorAll<HTMLButtonElement>(".model-choice")].every((button) => button.disabled));
  await act(async () => { query<HTMLButtonElement>(modelMenu, ".model-choice").click(); });
  assert.equal(chosen.length, 0);
  const codexFilter = [...modelMenu.querySelectorAll<HTMLButtonElement>(".model-provider-rail button")].find((button) => button.textContent?.includes("Codex"))!;
  await act(async () => { codexFilter.click(); });
  await act(async () => { item(modelMenu.querySelectorAll<HTMLButtonElement>("button.model-choice")[0]).click(); });
...
test("an engine that is signed out is greyed and inert, and its one sign-in button is what signs in", async () => {
  ...
  await act(async () => { query<HTMLButtonElement>(missing, ".model-choice").click(); });
...
test("a Claude behind the app offers only the models it knows, and says an upgrade brings back more", async () => {
  ...
  assert.deepEqual([...claude.querySelectorAll(".model-choice strong")].map((option) => option.textContent), ["Opus", "Sonnet"], "the models it never heard of are simply absent");
