...
import { parseWorkflowProgress, workflowProgressOf } from "../src/main/agent/workflow-progress.mts";
import {
  agentStateIn,
  formatElapsed,
  workflowAgentNote,
...
function workflow(agents: WorkflowAgent[], phases: WorkflowPhase[] = []): Workflow {
  return {
    id: "wf-1",
    name: "review-changes",
...
test("a workflow that ended took its unfinished agents with it", () => {
  const stopped: Workflow = { ...workflow([{ index: 0, label: "verify:a", state: "running" }]), status: "stopped", finishedAt: START + 10 };
