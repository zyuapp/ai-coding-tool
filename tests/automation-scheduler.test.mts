...
} from "../src/main/automation/automation-scheduler.mts";
import {
  isAutomation,
  isAutomationDraft,
  isAutomationPatch,
...
function memoryStore(initial: Automation[] = []) {
  ...
  return {
    ...
    saveAutomation: (automation: Automation) => { rows.set(automation.id, automation); },
    deleteAutomation: (id: string) => { rows.delete(id); },
  };
}
...
function schedulerFor(
  ...
  t.onTestFinished(() => scheduler.stop());
  return scheduler;
}
