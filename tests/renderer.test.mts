...
test("a side chat composes with everything the main composer has", async () => {
  ...
  const sideChatProps = (prompt: string, onPrompt: SideChatProps["onPrompt"]): SideChatProps => ({
    ...
    onPasteRecall() {},
    onPasteRemove() {},
    onFilesAdd() {}, onFileRecall() {}, onFileRemove() {},
    onImageRemove() {},
    onSend() {},
    onCancel() {},
...
test("the up arrow recalls sent prompts and the down arrow walks back to the draft", async () => {
  ...
  function Harness() {
    ...
    return renderTaskComposer({
      ...
      model: "opus",
      runActive: false,
      history: ["first question", "first question", "second question"].map((text) => ({ text, annotations: [], pastes: [], files: [] })),
      onPromptChange: setPrompt,
      onModeChange() {},
      onModelChange() {},
      queuedMessages: [],
...
test("the start options say where a thread begins, and searching narrows the branches", async () => {
  ...
  await act(async () => { item(projectOptions[1]).click(); });
