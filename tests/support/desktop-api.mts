...
import { engineDesktopStub, mobileDesktopStub } from "./mobile-desktop.mts";

export const automationView = (overrides: Partial<AutomationView> = {}): AutomationView => ({
  id: "automation-1",
  taskId: "task-1",
...
export type FakeDesktop = DesktopAPI & {
  sent: RunCommand[];
...
export function fakeDesktop(overrides: Partial<DesktopAPI> = {}): FakeDesktop {
  ...
  const api: DesktopAPI = {
    ...
    readAttachment: async () => "iVBORw0KGgo=", readAttachmentContext: async () => null, preserveMessageImages: async () => undefined, downloadImage: async () => undefined,
    ...
    setRemoteBrowserViewport: async () => {},
    controlBrowser: async () => {},
    readRemoteBrowserFrame: async () => null,
