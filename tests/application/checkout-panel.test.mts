...
test("the card counts its visible threads, including dismissed threads, and searches within the current checkout", () => {
  let state = fixture();
  const dismissed = reduce(state, { type: "task.dismiss", taskId: "build" });
  const view = required(deriveView(dismissed.state).checkout);
  assert.equal(view.threadCount, 2);
  assert.deepEqual(view.threads.map((thread) => thread.id), ["review", "build"]);
  state = reduce(state, { type: "checkout.search", query: "BUILD" }).state;
  assert.deepEqual(required(deriveView(state).checkout).threads.map((thread) => thread.id), ["build"]);
  assert.equal(required(deriveView(state).checkout).threadCount, 2);
  state = reduce(state, { type: "task.select", taskId: "local" }).state;
  assert.equal(required(deriveView(state).checkout).name, "Local");
  assert.deepEqual(required(deriveView(state).checkout).threads.map((thread) => thread.id), ["local"]);
});

test("destination search matches thread titles and branches and excludes other projects", () => {
  const state = fixture();
  state.worktrees.push({ ...heldWorktree("other"), projectId: "other", name: "Other project" });
  state.checkoutPanel = { open: true, mode: "move", query: "rounding", destination: null };
  assert.deepEqual(required(deriveView(state).checkout).destinations.map((item) => item.id), ["wt2"]);
  state.checkoutPanel.query = "feature/wt2";
  assert.equal(required(deriveView(state).checkout).destinations[0].name, "Payment fix");
  state.checkoutPanel.query = "other";
  assert.deepEqual(required(deriveView(state).checkout).destinations, []);
});
...
test("a new destination starts at the source checkout and moves only after creation succeeds", () => {
  ...
  assert.equal(required(deriveView(creating.state).checkout).transition, "Creating worktree…");
...
test("the session card loads deletion details without opening Settings and includes archived claimants", () => {
  ...
  assert.equal(required(deriveView(busy).checkout).deleteDisabled, true);
