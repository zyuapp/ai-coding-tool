...
test("a scheduled run declines when the task is archived, gone, or already running", () => {
  ...
  const declined = [
    workspace({ threads: [] }),
    workspace({ threads: [task("task-a", { archivedAt: 5 })] }),
    workspace({ threads: [task("task-a")], activeRuns: { "task-a": activeRun("task-a", "other") } }),
  ];
...
test("a scheduled run starts with its own framing and acknowledges the tick", () => {
  ...
  assert.equal(started.state.threads[0].messages[0].detail, "Automation run #2");
...
test("manual Sol compaction reuses the run lifecycle without becoming a task run", () => {
  ...
  assert.deepEqual(resolved.state.threads[0].messages, [], "compaction sends no user message");
  ...
  assert.equal(finished.state.threads[0].messages[0].text, "Context manual-compacted at 125,000 tokens.");
  assert.equal(finished.state.threads[0].outcome, "finished", "the prior task verdict survives context maintenance");
  assert.equal(finished.state.threads[0].runEndedAt, 5);
...
test("a Codex review picker starts a native review without adding a user message", () => {
  ...
  assert.deepEqual(resolved.state.threads[0].messages, []);

  const copied = { ...state, threads: [{ ...state.threads[0], inheritedContinuation: true as const }] };
  ...
  const claude = { ...state, threads: [{ ...state.threads[0], engine: "claude" as const, continuation: { provider: "claude" as const, value: "session-1" } }] };
...
test("a run that settles off screen flags its thread and refreshes its project", () => {
  const state = workspace({
    threads: [task("task-a", { projectId: "project-1" }), task("task-b")],
    projects: [{ id: "project-1", root: "/project", workspaceId: "workspace-1" }],
  ...
  assert.equal(opened.state.threads[0].outcomeUnread, undefined, "opening the thread takes the mark off");
  assert.equal(opened.state.threads[0].outcome, "finished", "but the verdict keeps its place in Priority");
  assert.deepEqual(deriveView(opened.state).activityThreads.priority.map((item) => item.id), ["task-a"]);
});

test("a dot can be dismissed without opening the thread it is on", () => {
  const state = workspace({ threads: [task("task-a", { outcome: "finished" }), task("task-b")], currentId: "task-b" });
...
test("one dismissal takes the dot off every thread carrying one", () => {
  ...
  assert.deepEqual(cleared.threads.map((item) => item.outcome), [undefined, undefined, undefined]);
...
test("the thread on screen ranks into priority unmarked, having been read as it settled", () => {
  ...
  assert.equal(next.threads[0].outcome, "finished");
  assert.equal(next.threads[0].outcomeUnread, undefined, "the user watched it end, so nothing marks it");
  assert.deepEqual(deriveView(next).activityThreads.priority.map((item) => item.id), ["task-a"], "and it still leaves the running list for priority");

  const behind = reduce({ ...watched, focused: false }, settle).state;
  assert.equal(behind.threads[0].outcomeUnread, undefined, "an unfocused window changes nothing; the thread is still the one on screen");

  const elsewhere = reduce({ ...watched, currentId: "task-b" }, settle).state;
  assert.equal(elsewhere.threads[0].outcomeUnread, true, "a thread the user is not on is marked");
});

test("selecting a thread takes its mark off and leaves it ranked", () => {
  const state = workspace({ threads: [task("task-a", { outcome: "failed", outcomeUnread: true }), task("task-b")], currentId: "task-b" });

  const { state: next } = reduce(state, { type: "task.select", taskId: "task-a" });

  assert.equal(next.threads[0].outcomeUnread, undefined);
  assert.deepEqual(deriveView(next).activityThreads.priority.map((item) => item.id), ["task-a"], "reading is not filing it away");

  const filed = reduce(next, { type: "task.dismiss", taskId: "task-a" });
  assert.equal(filed.state.threads[0].outcome, undefined);
  assert.deepEqual(deriveView(filed.state).activityThreads.priority, []);
});

test("a run the user stops keeps its thread in priority", () => {
  const state = workspace({
    threads: [task("task-a", { outcome: "finished" }), task("task-b")],
    currentId: "task-a",
  });
  ...
  assert.deepEqual(deriveView(started.state).activityThreads.running.map((item) => item.id), ["task-a"]);
  ...
  assert.equal(stopped.state.threads[0].outcome, "stopped");
  assert.equal(stopped.state.threads[0].outcomeUnread, undefined, "the user stopped it themselves, so nothing marks it");
  const view = deriveView(stopped.state);
  assert.deepEqual(view.activityThreads.priority.map((item) => item.id), ["task-a"], "it waits on the user rather than dropping into Threads");
  assert.deepEqual(view.activityThreads.running, []);
...
test("a run ending under an archived thread leaves no verdict on it", () => {
  const state = workspace({
    threads: [task("task-a", { archivedAt: 5 }), task("task-b")],
    activeRuns: { "task-a": activeRun("task-a", "run-1") },
    currentId: "task-b",
  });
  ...
  for (const status of ["cancelled", "succeeded", "failed"] as const) {
    ...
    assert.equal(settled.state.threads[0].outcome, undefined, `${status}: a thread already filed away is past ranking`);
    assert.equal(settled.state.threads[0].outcomeUnread, undefined, `${status}: and it never marks the app icon`);
  }
});

test("a new run supersedes the verdict of the one before it", () => {
  const state = run(workspace({ threads: [task("task-a", { outcome: "finished" })], currentId: "task-a" }), [
  ...
  assert.equal(started.state.threads[0].outcome, undefined, "the old verdict does not outlive the run it described");
  const view = deriveView(started.state);
  assert.deepEqual(view.activityThreads.priority, [], "a working thread is never in Priority");
  assert.deepEqual(view.activityThreads.running.map((item) => item.id), ["task-a"]);
});
...
test("a thread blocked on an approval leads until the user answers, then goes back to running", () => {
  ...
  assert.equal(asking.threads[0].outcome, undefined, "asking is live state, not a verdict written on the thread");
  assert.deepEqual(deriveView(asking).blockedThreadIds, new Set([taskId]));
  assert.deepEqual(deriveView(asking).activityThreads.priority.map((item) => item.id), [taskId]);
  assert.deepEqual(deriveView(asking).activityThreads.running, []);
  ...
  assert.deepEqual(deriveView(answered).activityThreads.priority, [], "the answered thread leaves no dot behind");
  assert.deepEqual(deriveView(answered).activityThreads.running.map((item) => item.id), [taskId]);
});
...
test("every visible thread lands in exactly one activity section", () => {
  ...
  const { priority, running, threads } = deriveView(state).activityThreads;
...
test("a streaming tail shows outside the task, so nothing half-written is ever stored", () => {
  ...
  assert.deepEqual(streaming.threads[0].messages, [], "a tail never becomes a message");
  assert.equal(streaming.threads[0], state.threads[0], "the task is untouched, so persistence has nothing to write");
  ...
  assert.equal(committed.threads[0].messages[0].text, "The first thing to check is the reducer.\n\n");
...
test("stopping a background process marks the row and asks the session, once", () => {
  ...
  const state = workspace({ threads: [task("task-a")], currentId: "task-a", activeRuns: { "task-a": activeRun("task-a", "run-a") }, backgroundProcesses: running });
  ...
  assert.deepEqual(reduce(workspace({ threads: [task("task-a")], currentId: "task-a" }), { type: "run.stop-process", processId: "bash-1" }).effects, [], "nothing running, nothing to stop");

  const shell = { id: "bash-1", kind: "shell" as const, description: "npm run dev" };
  const idle = workspace({ threads: [task("task-a")], currentId: "task-a" });
...
test("stopping a workflow reaches the thread's session after the run that started it has ended", () => {
  ...
  const state = workspace({ threads: [task("task-a")], currentId: "task-a", workflows: { "task-a": [workflow] } });
