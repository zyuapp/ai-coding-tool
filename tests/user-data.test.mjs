...
import { adoptUserDataFolder } from "../dist/main/main/user-data.js";
...
async function seedLegacy(root, registry) {
...
test("workspace roots inside the folder are repointed, and the ones outside it are left alone", async () => {
  ...
  const adopted = adoptUserDataFolder(root, NAME);
  const registry = JSON.parse(await readFile(path.join(adopted, "workspaces.v1.json"), "utf8"));
