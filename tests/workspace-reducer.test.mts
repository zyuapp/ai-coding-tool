...
import type { AutomationFire, ChangedFilesResult, CreatedWorktree, RunEvent } from "../src/contracts/ipc.ts";
import type { ViewPreferences } from "../src/contracts/preferences.ts";
import { sentPrompts, type Project, type Task, type TaskStoreData } from "../src/domain/task.ts";
...
function dock(state: WorkspaceState, owner?: string) {
...
function activeRun(taskId: string, runId: string, overrides: Partial<ActiveRun> = {}): ActiveRun {
  return {
    taskId,
    runId,
    sequence: 0,
...
function automation(taskId: string): AutomationView {
  return {
...
function effectOf<Type extends WorkspaceEffect["type"]>(
  transition: WorkspaceTransition,
  type: Type,
): Extract<WorkspaceEffect, { type: Type }> {
...
function required<Value>(value: Value | null | undefined, message = "expected value"): Value {
...
function correlatedRunEvent(taskId: string, runId: string, sequence: number, payload: RunEventPayload): WorkspaceInput {
...
function running(taskId = "task-a", runId = "run-a", overrides: Partial<WorkspaceState> = {}): WorkspaceState {
  return workspace({
    tasks: [task(taskId)],
    currentId: taskId,
...
function queueMessage(state: WorkspaceState, text: string, steer = false): WorkspaceState {
...
function projected(overrides: Partial<WorkspaceState> = {}): WorkspaceState {
...
function madeWorktree(id = "wt1"): CreatedWorktree {
...
function heldWorktree(id = "wt1"): Worktree {
...
function inside(worktree: Worktree, tasks: Task[]): Pick<WorkspaceState, "worktrees" | "tasks"> {
...
test("switching back to local hands the worktree back, and the thread records where the work went", () => {
  ...
  const note = required(released.state.tasks[0]?.messages.at(-1));
...
test("a checkout that lands after its thread is archived stays with that thread", () => {
  ...
  assert.equal(created.state.tasks[0].worktreeId, "wt1");
});
...
test("archiving a thread cancels its run and keeps its checkout", () => {
  ...
  assert.equal(archived.state.tasks[0].worktreeId, worktree.id);
...
test("manually deleting a worktree snapshots it and puts the thread back on the project", () => {
  ...
  assert.match(required(deleted.state.tasks[0]?.messages.at(-1)).text, /Worktree deleted/);
...
test("a thread started in a checkout the project already has runs there, and none is made", () => {
  ...
  const fresh = required(started.state.tasks.find((item) => item.id !== "task-a"));
...
test("threads sharing a checkout each fork their own session the first time they run in it", () => {
  ...
  assert.ok(required(first.state.tasks.find((item) => item.id === "task-b")).worktreeEnteredAt);
  assert.equal(required(first.state.tasks.find((item) => item.id === "task-a")).worktreeEnteredAt, 3, "and says nothing about the thread already in there");
});
...
test("leaving a checkout another thread is still in takes only this thread's claim", () => {
  ...
  assert.match(required(leaving.state.tasks[0]?.messages.at(-1)).text, /other threads are still working in it/);
...
test("deleting a checkout puts every thread in it back on the project", () => {
  ...
  assert.match(required(deleted.state.tasks[1]?.messages.at(-1)).text, /Worktree deleted/);
});
...
test("a workflow panel closes once the record it was following is gone", () => {
  ...
  const dropped = reduce(opened, { type: "workflow.event", event: { type: "workflow.finished", taskId: "task-1", id: "wf-1", status: "completed", summary: "Review completed" } });
