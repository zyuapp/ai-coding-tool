...
import { activitySections, backfillSortIndex, moveTask, nextSortIndex, orderTasks } from "../src/application/task-order.ts";
...
test("running holds the sidebar's order, so a thread speaking never moves the rows", () => {
  ...
  const spoke: Task[] = [tasks[0], { ...tasks[1], messages: [{ id: "m", kind: "assistant", text: "working", at: 500 }] }];
