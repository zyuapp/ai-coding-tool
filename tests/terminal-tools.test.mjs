...
import { terminalTools } from "../dist/main/main/agent/terminal-tools.mjs";
...
function fakeBridge(result = { kind: "snapshot", snapshot: snapshot() }) {
  const calls = [];
  return {
    calls,
...
test("a bridge that fails answers as an error rather than throwing at the agent", async () => {
  const failing = { read: async () => { throw new Error("the window is not open"); } };
