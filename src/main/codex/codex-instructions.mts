...
import { coordinationInstructions } from "../agent/coordination-instructions.mjs";
...
export function codexInstructions(channel: RunChannel, coordinationRole?: CoordinationRole) {
  return [DEVELOPER_INSTRUCTIONS, ...coordinationInstructions(coordinationRole), ...(channel === "side" ? [SIDE_CHAT_INSTRUCTIONS] : [])].join("\n\n");
}
