...
const threadInstructions = `AICodingTool holds the user's other threads, and the aicodingtool-threads tools are the only way to reach them. Read them with list_threads and read_thread when the user points at other, recent, or related work instead of guessing from memory. Start a thread per piece of work when the user asks for several things to run side by side, and give each one a prompt that stands alone: a new thread inherits none of this conversation. Wait on a thread you started with wait_for_thread rather than polling read_thread. Archiving or stopping a thread throws away work in progress, so only do it when the user asked for it. When you name another thread in an answer, link it as [title](aicodingtool://thread/<id>) so the user can open it from your message. A message that already carries one of those links is naming a thread for you, so read it rather than searching for it.`;
const automationInstructions = `This task can schedule itself. When the user asks to repeat, babysit, poll, or watch something on a cadence, use the aicodingtool-automation tools instead of looping yourself or reaching for cron. An automation runs the same prompt on its own schedule with no user present, so write the prompt to stand alone and carry its own stop condition. When a scheduled run is what is executing and that stop condition is met, call the stop tool; nothing else ends an automation. A schedule that only matters when it turns something up takes a surfaceWhen sentence, and its runs then have to say what they found with notify or say they found nothing with nothing_to_report; a schedule without one reaches the user on every run.`;
  ...
  private options(input: ProviderRunInput, prompt: AsyncIterable<SDKUserMessage>, canUseTool: CanUseTool) {
    ...
    if (input.computerUse.status === "available") {
      ...
      mcpServers["aicodingtool-computer-use"] = createSdkMcpServer({
        ...
        version: "1.0.0",
        alwaysLoad: true,
        tools: [tool("request_setup", "Use when a task requires operating another application's interface but computer use needs to be enabled in AICodingTool.", {}, async () => {
