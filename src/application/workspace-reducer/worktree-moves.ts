...
import { now, rereadDiff, settled, targetId, threadBusy, withCreatingWorktree, WORKTREE_CREATING_ERROR, WORKTREE_MISSING_ERROR, WORKTREE_PROJECT_ERROR, WORKTREE_RELEASING_ERROR, WORKTREE_RUNNING_ERROR } from "./shared.js";
...
export function relocateThread(state: WorkspaceState, thread: Thread, destination: Worktree | undefined): WorkspaceTransition {
  ...
  if (next.currentId === thread.id) next.checkoutPanel = { ...next.checkoutPanel, mode: "threads", query: "", destination: null };
...
export function reduceWorktreeMove(state: WorkspaceState, input: Extract<TaskCommand, { type: "task.move-worktree" }>): WorkspaceTransition {
  ...
  if (!thread || !project?.workspaceId) return settled({ ...state, actionError: WORKTREE_PROJECT_ERROR });
  ...
  if (threadBusy(state, thread.id)) return settled({ ...state, actionError: WORKTREE_RUNNING_ERROR });
  ...
  if (!worktree) return settled({ ...state, actionError: WORKTREE_MISSING_ERROR });
  if (worktree.projectId !== project.id) return settled({ ...state, actionError: "Choose a worktree in this thread's project." });
  ...
  if (state.deletingWorktrees.includes(worktree.root) || state.threads.some((item) => item.worktreeId === worktree.id && state.releasingWorktrees.includes(item.id))) {
    return settled({ ...state, actionError: WORKTREE_RELEASING_ERROR });
  }
  if (state.managedWorktrees !== null) {
    const managed = state.managedWorktrees.find((item) => item.root === worktree.root);
    if (!managed?.repository) return settled({ ...state, actionError: WORKTREE_MISSING_ERROR });
  }
