...
export function reduceSending(state: WorkspaceState, input: SendInput): WorkspaceTransition {
  switch (input.type) {
    ...
    case "question.answer": {
      ...
      if (!active || active.runId !== input.runId || !question) return input.type === "question.set-answer" ? settled(state) : rejected(state, "That question is no longer waiting for an answer.");
      if (question.submitting) return settled(state);
      if (input.type === "question.set-answer") {
        if (question.answer === input.text) return settled(state);
        return settled({ ...state, activeRuns: { ...state.activeRuns, [input.taskId]: { ...active, questions: active.questions?.map((pending) => pending === question ? { ...pending, answer: input.text } : pending) } } });
      }
      const text = (input.text ?? question.answer ?? "").trim();
      if (!text) return settled(state);
      const next = { ...state, activeRuns: { ...state.activeRuns, [input.taskId]: { ...active, questions: active.questions?.map((pending) => pending === question ? { ...pending, answer: text, submitting: true } : pending) } } };
      return settled(withAttendedRun(next, input.taskId), [{ type: "send-run-command", command: { type: "answer-question", taskId: input.taskId, runId: input.runId, requestId: input.requestId, questionId: input.questionId, text } }]);
    }
    case "task.send": {
