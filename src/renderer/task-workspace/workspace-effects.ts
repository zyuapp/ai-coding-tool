...
export async function runWorkspaceEffect(effect: WorkspaceEffect, host: EffectHost): Promise<void> {
  switch (effect.type) {
    ...
    case "file.open": case "app.open-folder": case "app.check-for-updates": case "app.open-source-licenses": case "browser.open": case "browser.navigate":
    ...
    case "engine.reload-settings": case "engine.read": case "engine.sign-in":
