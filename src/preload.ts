...
const api: DesktopAPI = {
  ...
  saveAttachment: (data: string, original?: string) => ipcRenderer.invoke("attachment:save", data, original),
  readAttachment: (file: string) => ipcRenderer.invoke("attachment:read", file),
  readAttachmentContext: (file: string) => ipcRenderer.invoke("attachment:context", file),
  ...
  setRemoteBrowserViewport: (tabId, viewport) => ipcRenderer.invoke("browser:remote-viewport", tabId, viewport),
  controlBrowser: (tabId, epoch, input) => ipcRenderer.invoke("browser:control", tabId, epoch, input),
  readRemoteBrowserFrame: (computerId, tabId) => ipcRenderer.invoke("browser:remote-frame", computerId, tabId),
  configureBrowserPermissions: (permissions) => ipcRenderer.invoke("browser:permissions", permissions),
  openBrowserTab: (tabId: string, url?: string, taskId?: string, offscreen?: boolean) => ipcRenderer.invoke("browser:open", tabId, url, taskId, offscreen),
